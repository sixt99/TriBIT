...
cudaError_t do_cudaMalloc_log(T** devPtr, size_Type size, std::string label = "", bool verbose = false) {
    cudaError_t status = cudaMallocFromPoolAsync(devPtr, size, g_pool, g_stream);
        ...
        if (verbose) {
                ...
                std::cout << "\033[31m" << label << " " << (void*)*devPtr << "\033[0m " << "Allocating " << size << " bytes, \033[31m" << size / (1024.0 * 1024.0 * 1024.0)
                                  << "\033[0m GB on GPU\n";
        }
    return status;
}
...
__global__ void compute_binary_popcount_mma_9(
    ...
    // Fill dA
        ...
        if (threadIdx.x < 8 && threadIdx.y < 4) {
                ...
                } else dA[4 * threadIdx.x + threadIdx.y] = 0;
        }
...
__global__ void compute_n_warps_per_i(uint64_t * d_warps_per_i, uint32_t * d_set_ptr_u, uint32_t * popc_array, uint32_t size) {
        uint64_t idx = (uint64_t) blockIdx.x * (uint64_t) blockDim.x + (uint64_t) threadIdx.x;
        if (idx < size) {
                d_warps_per_i[idx] = (uint64_t) (d_set_ptr_u[idx + 1] - d_set_ptr_u[idx]) * (uint64_t) ((popc_array[idx + 1] - popc_array[idx] + 31) / 32);
        }
}
...
extern "C" uint64_t count_partial_triangles_off_diagonal(
        uint64_t ** cols_rows_zipped_COLS,
        uint64_t * nnz_COLS_ptr,
        uint32_t num_ptrs_cols,
        uint64_t ** cols_rows_zipped_IDXS,
        uint64_t * nnz_IDXS_ptr,
        uint32_t num_ptrs_idxs,
        uint64_t num_nodes
) {
        ...
        for (int i = 0; i < num_ptrs_cols; ++i) {
                ...
                cudaMemcpy(d_cols_rows_zipped_COLS + offset, cols_rows_zipped_COLS[i], nnz_block * sizeof(uint64_t), cudaMemcpyHostToDevice);
        }
        ...
        for (int i = 0; i < num_ptrs_idxs; ++i) {
                ...
                cudaMemcpy(d_cols_rows_zipped_IDXS + offset, cols_rows_zipped_IDXS[i], nnz_block * sizeof(uint64_t), cudaMemcpyHostToDevice);
        }
        ...
        // This turns out to be very useful, because d_idxs do not need ALL the ones, only those reaching the lowest block of cols
        ...
        compute_histogram_popc<<<blocks(nnz_IDXS,256),256>>>(
                d_cols_rows_zipped_IDXS,
                popc_array,
                nnz_IDXS
        );
