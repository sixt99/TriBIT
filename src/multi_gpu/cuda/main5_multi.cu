...
__global__ void compute_binary_popcount_mma_9(const uint32_t* d_mask_u, T* d_col_id_u, T* d_set_ptr_u,
                                              ...
                                              uint32_t compressed_size_u, uint64_t* d_n_blocks_per_row,
                                              uint32_t non_zero_rows, uint64_t total_num_warps, uint32_t* d_i_values) {
...
__global__ void compute_n_blocks_per_row(uint64_t* popc_long, uint32_t* d_set_ptr_u, uint32_t* popc_array,
                                         uint32_t non_zero_rows, uint32_t* d_i_values, uint32_t size) {
  uint64_t idx = (uint64_t)blockIdx.x * (uint64_t)blockDim.x + (uint64_t)threadIdx.x;
  if (idx < size) {
...
extern "C" void init_gpu_kernel5(int gpu_id) {
...
extern "C" uint64_t count_partial_triangles_diagonal(uint64_t** cols_rows_zipped, uint64_t* nnz_ptr, uint32_t num_ptrs,
                                                     uint64_t num_nodes) {
  uint64_t* d_cols_rows_zipped;
  uint64_t nnz = nnz_ptr[num_ptrs];  // Safe since nnz_ptr is cumulative and has one extra value
  cudaMalloc_log(&d_cols_rows_zipped, nnz * sizeof(uint64_t));
  for (int i = 0; i < num_ptrs; ++i) {
    uint64_t offset = nnz_ptr[i];
    uint64_t nnz_block = nnz_ptr[i + 1] - nnz_ptr[i];
    cudaMemcpy(d_cols_rows_zipped + offset, cols_rows_zipped[i], nnz_block * sizeof(uint64_t), cudaMemcpyHostToDevice);
  }
  ...
  // -------- MATRIX L -------- //
  // uint64_t nnz_l = nnz_ptr[num_ptrs] - nnz_ptr[num_ptrs - 1]; // L only needs last part
  uint32_t* d_mask_l;
  cudaMalloc_log(&d_mask_l, nnz * sizeof(uint32_t));
  create_non_compact_mask_l<<<blocks(nnz, 256), 256>>>(d_cols_rows_zipped, d_mask_l,
                                                       nnz);  // + nnz_ptr[num_ptrs - 1], d_mask_l, nnz_l);
  // create global_idxs
  find_global_idxs<<<blocks(nnz, 256), 256>>>(d_cols_rows_zipped, global_idxs,
                                              nnz);  // + nnz_ptr[num_ptrs - 1], global_idxs, nnz);
  ...
  cudaMemset(d_set_ptr_l, 0, N / 32 * sizeof(uint32_t));  // Last element will be overwritten by complete_sum
  ...
  uint32_t non_zero_rows =
      run_length_encode(  // We use RLE instead of histogram to ignore ZEROs in rows (which are completely unnecessary)
          thrust::make_transform_iterator(global_idxs, divide_by_N_32()),  // Compressed rows (no need to store them)
          d_set_ptr_u, compressed_size_u);
  cudaFree_log(global_idxs);
  ...
  complete_sum(thrust::make_transform_iterator(d_mask_u, Popc_Custom()), popc_long, compressed_size_u);
  // Create popc_array
  uint32_t* popc_array;
  ...
  compute_n_blocks_per_row<<<blocks(compressed_size_u, 256), 256>>>(popc_long, d_set_ptr_u, popc_array, non_zero_rows,
                                                                    d_i_values, compressed_size_u);
  complete_sum(popc_long, popc_long, compressed_size_u);
  ...
  for (uint32_t launch = 0; launch < number_of_launches; ++launch) {
    ...
    compute_binary_popcount_mma_9<<<blocksPerGrid, threadsPerBlock>>>(
        ...
        compressed_size_u, d_n_blocks_per_row, non_zero_rows, total_num_warps, d_i_values);
