...
extern "C" uint64_t count_partial_triangles_diagonal(
        uint64_t ** cols_rows_zipped,
        uint64_t * nnz_ptr,
        uint32_t num_ptrs,
        uint64_t num_nodes
) {
        ...
        create_non_compact_mask_l<<<blocks(nnz,256),256>>>(d_cols_rows_zipped, d_mask_l, nnz);// + nnz_ptr[num_ptrs - 1], d_mask_l, nnz_l);
        // create global_idxs
        find_global_idxs<<<blocks(nnz,256),256>>>(d_cols_rows_zipped, global_idxs, nnz);// + nnz_ptr[num_ptrs - 1], global_idxs, nnz);
