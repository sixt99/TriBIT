...
__global__ void flag_non_singletons(const uint64_t* d_in, out_T* d_mask, const uint64_t size) {
  ...
  if (idx < size) {
    if (idx == 0)
      d_mask[idx] = (d_in[idx] >> 32) == (d_in[idx + 1] >> 32);
    else if (idx == (size - 1))
      d_mask[idx] = (d_in[idx - 1] >> 32) == (d_in[idx] >> 32);
    else
      d_mask[idx] = ((d_in[idx - 1] >> 32) == (d_in[idx] >> 32)) || ((d_in[idx] >> 32) == (d_in[idx + 1] >> 32));
  }
}
...
uint32_t count_triangles(std::vector<uint64_t> cols_rows_zipped, uint32_t nnz, Config config, bool print_info) {
  ...
  if (number_of_launches > 1) throw std::logic_error("Not implemented: total_num_blocks is bigger than INTMAX_32");
  ...
  if (print_info) {
    printf(
        ...
        "Triangles          : %llu\n"
        "============================================================\n",
        config.input_file.c_str(), total_num_blocks, ms_preprocessing, ms, (unsigned long long)triangle_count);
